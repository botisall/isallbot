...
let loveyou = lovou[Math.floor(Math.random() * lovou.length)]
    ...
    let helloisall = fs.readFileSync('./mp3/PTT-20211218-WA0243.opus') 
    conn.sendMessage(m.chat, {
        react: {
          text: `${loveyou}`,
          key: m.key,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   /* let hiai = /(hi|hii|hiii|hi isall|hii isall|hiii isall|hy|halo|hallo|helo|hello|hy isall|halo isall|hallo isall|helo isall|hello isall)/i
...
let hiisall = hiain[Math.floor(Math.random() * hiain.length)]
    if (isAih && !m.fromMe) {
